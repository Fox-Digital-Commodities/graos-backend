...
export class KanbanService {
  ...
  async deleteColumn(columnId: string) {
    ...
    const cardsCount = await this.kanbanCardRepository.count({
      where: { columnId },
    });

    if (cardsCount > 0) {
      throw new BadRequestException(
        `Não é possível excluir coluna com ${cardsCount} cards`,
      );
    }
  ...
  async moveCard(
    ...
    if (oldColumnId !== targetColumnId) {
      const oldColumn = await this.kanbanColumnRepository.findOne({
        where: { id: oldColumnId },
      });
      const newColumn = await this.kanbanColumnRepository.findOne({
  ...
  private async adjustCardPositions(
    columnId: string,
    fromPosition: number,
    adjustment: number,
  ) {
  ...
  private async trackCardChanges(oldCard: KanbanCard, newCard: KanbanCard) {
    ...
    if (oldCard.assigneeId !== newCard.assigneeId) {
      changes.push({
        type: 'assigned',
        description: newCard.assigneeId
          ? 'Card atribuído'
          : 'Atribuição removida',
  ...
  async getBoardStats(boardId: string) {
    ...
    const totalCards = await this.kanbanCardRepository.count({
